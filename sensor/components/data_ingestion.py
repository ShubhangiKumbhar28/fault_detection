...
class DataIngestion:
    ...
    def initiate_data_ingestion(self)->artifact_entity.DataIngestionArtifact:
        try:
            ...
            logging.info(f"Save data into feature store")
            ##replace na with NAN
            ...
            train_df,test_df=train_test_split(df,test_size=self.data_ingetion_config.test_size,random_state=42)
            ...
            ##Save df to feature store folder
            df.to_csv(path_or_buf=self.data_ingetion_config.train_file_path,index=False,header=True)
            df.to_csv(path_or_buf=self.data_ingetion_config.test_file_path,index=False,header=True)
